...
echo'
    <section class="container_box">
      <div class="row">';
      if(isset($_GET['from']) OR isset($_GET['to'])){
        echo'<h3 class="text-center">LAPORAN DETAIL HARIAN PERSONIL SAT LANTAS POLRESTA PEKANBARU<br>PERIODE WAKTU '.tanggal_indo($_GET['from']).' - '.$_GET['to'].'</h3>';}
        else{
        echo'<h3 class="text-center">LAPORAN DETAIL BULAN<br>'.tanggal_indo($month).' - '.$year.'</h3>';
       }
        echo'
        <p>Nama Personil  : '.$row['employees_name'].'</p>
...
echo'
    <section class="container_box">
      ...
      if(isset($_GET['from']) OR isset($_GET['to'])){
        ...
        echo'
        <p>Nama  Personel : '.$row['employees_name'].'</p>
        <p>Jabatan : '.$row['position_name'].'</p><br>
      <div class="content_box">
        <table class="customTable">
          <thead>
